...
class ExponentialGuesser {

public:
  ExponentialGuesser(const probability_t gamma, const probability_t p_low)
  ...
  void below() {
    if (binary_search) {
      ...
      LOG_DEBUG("[ExponentialGuesser] Below in binary search (new m_lower" << m_lower << ")");
    } else {
      LOG_DEBUG("[ExponentialGuesser] Below in exponential search: start binary search");
  ...
  void above() {
    if (binary_search) {
      ...
      LOG_DEBUG("[ExponentialGuesser] Above in binary search (new m_upper" << m_lower << ")");
    } else {
      m_upper = m_lower;
  ...
  bool stop() const {
    if (binary_search) {
      LOG_DEBUG("[ExponentialGuesser] Stopping condition: " << (1.0-m_lower/m_upper) << "<=" << m_gamma);
    }
...
class ExtendedExponentialGuesser {

public:
  ExtendedExponentialGuesser(const probability_t gamma, const probability_t p_low)
      : m_p_low(p_low), m_gamma(gamma), m_upper(1.0), m_lower(1.0),
        binary_search(false), m_i(0) {}
  ...
  void below() {
    if (binary_search) {
      m_lower = (m_upper + m_lower) / 2;
      LOG_DEBUG("[ExtendedExponentialGuesser] Below in binary search (new m_lower" << m_lower << ")");
    } else {
      LOG_DEBUG("[ExtendedExponentialGuesser] Below in exponential search: start binary search");
      binary_search = true; // Start binary search on the next call of `guess`
    }
  }
  ...
  void above() {
    if (binary_search) {
      m_upper = (m_upper + m_lower) / 2;
      LOG_DEBUG("[ExtendedExponentialGuesser] Above in binary search (new m_upper" << m_lower << ")");
    } else {
      ...
      LOG_DEBUG("[ExtendedExponentialGuesser] Above in exponential search. New m_lower " << m_lower << " new m_upper " << m_upper);
    }
  }
  ...
  bool stop() const {
    if (binary_search) {
      LOG_DEBUG("[ExtendedExponentialGuesser] Stopping condition: " << (1.0-m_lower/m_upper) << "<=" << m_gamma);
    }
    return binary_search && (1.0-m_lower/m_upper) <= m_gamma;
  }
